    ...
    public partial class UCTreeView : UserControl
    {
        ...
        private void Init()
        {
            try
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    struct_DeviceEnable = struct_DeviceEnables[i];
                    if (struct_DeviceEnable.enable)
                    {
                        int deviceNo = Convert.ToInt32(struct_DeviceEnable.ipEndPoint.Address.ToString().Split('.')[3]);//启用的设备编号
                        list_EnableDeviceNo.Add(deviceNo);
                    }
                }
