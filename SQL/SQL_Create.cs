...
namespace OpticalFiber
{
    /// <summary>
    /// 检查数据库 创建数据库 数据表
    /// 直接使用root账户进行操作  应该为 其创建单独的用户与数据库  然后授予用户最小权限   操作需要有点专业知识 
    /// </summary>
    public class SQL_Create
    {
        ...
        private bool IsDBExit(string dbName)
        {
            ...
            try
            {
                string connstr = "data source =localhost;user id=root;password=adsensor;pooling=false;charset=utf8";
        ...
        private bool Create_Table_Pwd()
        {
            ...
            try
            {
                ...
                string sql = "create table pwd(userName varchar(20) ,passWord varchar(32))";
                conn = new MySqlConnection(connstr);
                cmd = new MySqlCommand(sql, conn);
                conn.Open();
                cmd.ExecuteNonQuery();
        ...
        private bool Create_Table_device()
        {
            ...
            {
                string connstr = "data source =localhost;database=SM2003;user id=root;password=adsensor;pooling=false;charset=utf8";
                string sql = "create table device(deviceNo tinyint unsigned,deviceName varchar(20),ipAddress varchar(15),port smallint,enable tinyint(1))";
                conn = new MySqlConnection(connstr);
                cmd = new MySqlCommand(sql, conn);
                conn.Open();
                cmd.ExecuteNonQuery();
        ...
        private bool Create_Table_prjName()
        {
            ...
            {
                string connstr = "data source =localhost;database=SM2003;user id=root;password=adsensor;pooling=false;charset=utf8";
                string sql = "create table prjname(prjId  tinyint unsigned,prjName varchar(32))";
                conn = new MySqlConnection(connstr);
                cmd = new MySqlCommand(sql, conn);
                conn.Open();
                cmd.ExecuteNonQuery();
        ...
        private bool Create_Table_prtName()
        {
            ...
            {
                string connstr = "data source =localhost;database=SM2003;user id=root;password=adsensor;pooling=false;charset=utf8";
                string sql = "create table prtName(deviceNo tinyint unsigned,channelNo tinyint unsigned,prtNo tinyint unsigned,prtName varchar(32))";
                conn = new MySqlConnection(connstr);
                cmd = new MySqlCommand(sql, conn);
                conn.Open();
                cmd.ExecuteNonQuery();
        ...
        private bool Create_Table_channel()
        {
            ...
            {
                string connstr = "data source =localhost;database=SM2003;user id=root;password=adsensor;pooling=false;charset=utf8";
                string sql = "create table channel(deviceNo tinyint unsigned,channelNo tinyint unsigned,channelName varchar(32),locationX double,locationY double,isEnable tinyint(1))";
                conn = new MySqlConnection(connstr);
                cmd = new MySqlCommand(sql, conn);
                conn.Open();
                cmd.ExecuteNonQuery();
        ...
        private bool Create_Table_PagePic()
        {
            ...
            try
            {
                ...
                string connstr = "data source =localhost;database=SM2003;user id=root;password=adsensor;pooling=false;charset=utf8";
                string sql = "create table pagePic(picNo tinyint unsigned,pic mediumblob)";
                conn = new MySqlConnection(connstr);
                cmd = new MySqlCommand(sql, conn);
                conn.Open();
                cmd.ExecuteNonQuery();
        ...
        private bool Create_Table_alarm()
        {
            ...
            {
                string connstr = "data source =localhost;database=SM2003;user id=root;password=adsensor;pooling=false;charset=utf8";
                string sql = "create table alarm(dateTime datetime,deviceNo tinyint unsigned,channelNo tinyint unsigned,partitionNo tinyint unsigned,position smallint,illustrate varchar(32),relay tinyint unsigned,type varchar(10),alarmvalue double,threshold double,primary key(datetime,deviceNo,channelNo,partitionNo))";
                conn = new MySqlConnection(connstr);
                cmd = new MySqlCommand(sql, conn);
                conn.Open();
                cmd.ExecuteNonQuery();
        ...
        private bool Create_Table_audit()
        {
            ...
            {
                string connstr = "data source =localhost;database=SM2003;user id=root;password=adsensor;pooling=false;charset=utf8";
                string sql = "create table audit(dateTime datetime,user tinyint unsigned,record varchar(32))";
                conn = new MySqlConnection(connstr);
                cmd = new MySqlCommand(sql, conn);
                conn.Open();
                cmd.ExecuteNonQuery();
